...
namespace GameRebellionSdk.Unity.Adapters
{
    ...
    /// </summary>
    internal sealed class AndroidAdapter : IGameRebellionAdapter
    {
#if UNITY_ANDROID && !UNITY_EDITOR
        // At Android build time, the full GameRebellion.Core.dll is available
        ...
        public int Initialize(GrConfig config)
        {
            try
            {
                if (config == null)
                {
                    UnityEngine.Debug.LogError("[AndroidAdapter] Config is null");
                    return -2;
                }

                // Initialize the native Android adapter FIRST (required for device info)
                if (!InitializeNativeAdapter())
                {
                    UnityEngine.Debug.LogWarning("[AndroidAdapter] Native adapter init failed, continuing with fallback device info");
                }

                // Set cache directory to Unity's persistent data path (required for Android)
                var persistentPath = UnityEngine.Application.persistentDataPath;
                UnityEngine.Debug.Log($"[AndroidAdapter] Setting cache directory: {persistentPath}");
                global::GameRebellionSdk.Core.GameRebellionUnityAPI.SetCacheDirectory(persistentPath);

                UnityEngine.Debug.Log("[AndroidAdapter] Converting environment...");
            ...
            {
                UnityEngine.Debug.LogError($"[AndroidAdapter] Initialize failed: {ex.GetType().Name}: {ex.Message}");
                UnityEngine.Debug.LogError($"[AndroidAdapter] Stack trace: {ex.StackTrace}");
                if (ex.InnerException != null)
                {
                    UnityEngine.Debug.LogError($"[AndroidAdapter] Inner exception: {ex.InnerException.GetType().Name}: {ex.InnerException.Message}");
                    UnityEngine.Debug.LogError($"[AndroidAdapter] Inner stack trace: {ex.InnerException.StackTrace}");
                }
                return -1;
            }
        }
        ...
        /// <summary>
        /// Initializes the native Android adapter (GRAndroidNativeAdapter.java) via JNI.
        /// This must be called BEFORE SDK initialization so the native library can provide device info.
        /// </summary>
        private bool InitializeNativeAdapter()
        {
            try
            {
                UnityEngine.Debug.Log("[AndroidAdapter] Initializing native adapter...");

                using (var unityPlayer = new UnityEngine.AndroidJavaClass("com.unity3d.player.UnityPlayer"))
                {
                    if (unityPlayer == null)
                    {
                        UnityEngine.Debug.LogError("[AndroidAdapter] Failed to get UnityPlayer class");
                        return false;
                    }
                    ...
                    using (var activity = unityPlayer.GetStatic<UnityEngine.AndroidJavaObject>("currentActivity"))
                    {
                        if (activity == null)
                        {
                            UnityEngine.Debug.LogError("[AndroidAdapter] Failed to get currentActivity");
                            return false;
                        }
                        ...
                        using (var context = activity.Call<UnityEngine.AndroidJavaObject>("getApplicationContext"))
                        {
                            if (context == null)
                            {
                                UnityEngine.Debug.LogError("[AndroidAdapter] Failed to get application context");
                                return false;
                            }

                            using (var adapterClass = new UnityEngine.AndroidJavaClass("com.gamerebellion.adapter.GRAndroidNativeAdapter"))
                            {
                                if (adapterClass == null)
                                {
                                    UnityEngine.Debug.LogError("[AndroidAdapter] Failed to find GRAndroidNativeAdapter class. Is the AAR included?");
                                    return false;
                                }
            ...
            {
                UnityEngine.Debug.LogError($"[AndroidAdapter] Native adapter initialization failed: {ex.GetType().Name}: {ex.Message}");
                UnityEngine.Debug.LogError($"[AndroidAdapter] Stack trace: {ex.StackTrace}");
                return false;
            }
        }
        ...
        public string GetLastError() => "AndroidAdapter not available on this platform";
